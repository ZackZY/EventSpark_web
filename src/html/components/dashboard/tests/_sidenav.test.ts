...
describe('Side Nav Bar', () => {
    let html;
    beforeAll(() => {
        html = fs.readFileSync(path.resolve(__dirname, '../_sidenav.astro'), 'utf8');
    });
    ...
    test('should render the navigation list with correct items', () => {
        ...
        expect(navItems[1].querySelector('p').textContent.trim()).toBe('Administrator');
        expect(navItems[2].querySelector('p').textContent.trim()).toBe('Analytics');
    });
